...
import meshio
import numpy as np
from Python_module_Quang import check_create_dir

# -------------------- Extract file extension -------------------- #
# filePath = 'Rectangular_2.msh'
# filePath = sys.argv[1]

def gmsh_xml_converter_2D(filePath):
    start_time = timeit.default_timer()
    
    # -------------------- Read mesh -------------------- #
    fileName, fileExtension = os.path.splitext(filePath)
    path_root = os.getcwd()
    filePath = os.path.join(path_root, filePath)
    ...
    print(msh.cell_data)
    print(type(msh.cell_data))
    # -------------------- Convert to xdmf files -------------------- #
    check_create_dir('xdmf')
    meshio.write(
        f"xdmf/{fileName}.xdmf",
        meshio.Mesh(points,
                    cells={element_types[0]: msh.cells_dict[element_types[0]]}))
    meshio.write(
        f"xdmf/{fileName}_physical_region.xdmf",
        meshio.Mesh(
            points,
            cells={element_types[0]: msh.cells_dict[element_types[0]]},
            cell_data={"name_to_read": [msh.cell_data["gmsh:physical"][0]]}))
    meshio.write(
        f"xdmf/{fileName}_facet_region.xdmf",
        meshio.Mesh(
            points,
            cells={element_types[0]: msh.cells_dict[element_types[0]]},
            cell_data={"name_to_read":
                    [msh.cell_data["gmsh:physical"][0]]}))  # gmsh:geometrical

    # -------------------- Reload xdmf files -------------------- #
    mesh = Mesh()
    with XDMFFile(f"xdmf/{fileName}.xdmf") as infile:
        infile.read(mesh)

    # Physical (subdomain) regions
    mvc_block = MeshValueCollection("size_t", mesh, dim)
    with XDMFFile(f"xdmf/{fileName}_physical_region.xdmf") as infile:
    ...
    with XDMFFile(f"xdmf/{fileName}_facet_region.xdmf") as infile:
        infile.read(mvc_facet, "name_to_read")
    boundaries = MeshFunction("size_t", mesh, mvc_facet)

    cell_data = msh.cell_data["gmsh:physical"][0]
