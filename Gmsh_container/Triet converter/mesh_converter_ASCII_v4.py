...
# -------------------- Reload xdmf files -------------------- #
mesh = Mesh()
with XDMFFile(f"xdmf/{filePath}.xdmf") as infile:
    infile.read(mesh)

# Physical (subdomain) regions
mvc_block = MeshValueCollection("size_t", mesh, dim)
with XDMFFile(f"xdmf/{filePath}_physical_region.xdmf") as infile:
...
with XDMFFile(f"xdmf/{filePath}_facet_region.xdmf") as infile:
